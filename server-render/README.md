...
| `MUSIC_PROVIDER` | Optional, defaults to `elevenlabs`. `elevenlabs` uses ElevenLabs Music (`POST /v1/music`) — one synchronous request, instrumental **and** vocals, up to 10 minutes per run. `replicate` selects the MusicGen path (instrumental-only, much shorter usable clips); a vocals request still prefers ElevenLabs when a key exists, since MusicGen cannot sing. `suno` opts into the third-party Suno path for vocals only (instrumentals still go to ElevenLabs), and only when a Suno key is also configured — otherwise it falls back to ElevenLabs rather than failing the request. See `pickProvider` in `music.js` for the exact resolution; `base44/functions/submitMusic/entry.ts` keeps a hand-synced copy of it (guarded by `test/musicProvider.test.js`). |
...
| `SUNO_API_KEY` | Optional. Required only to actually use `MUSIC_PROVIDER=suno`. Suno has no official public API — this targets the de facto third-party reseller contract; see the Suno section of `music.js`. |
| `SUNO_API_BASE_URL` | Optional, defaults to `https://api.sunoapi.org`. Point at a different Suno reseller. |
| `SUNO_MODEL` | Optional, defaults to `V4_5`. |
...
- `POST /music` — body is `{ prompt, durationSeconds, instrumental, lyrics, genre, mood, title, model_version }` (see `music.js`). Requires `x-render-secret`. Returns `202 { jobId }` immediately. `instrumental` defaults to `true`; pass `false` with `lyrics` to ask for a sung track. The finished job carries `vocals: true|false` saying whether one was actually produced — false means the request could only be served by MusicGen, which has no vocal synthesis. `durationSeconds` is clamped per provider: 3–600s on ElevenLabs, 5–30s on Replicate. `model_version` is Replicate-only; `genre`/`mood`/`title` are used by Suno.
